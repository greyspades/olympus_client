...
export const ArticlePreview = ({article}: PreviewProps) => {
    ...
    const renderSection1 = () => (
        <div className='relative w-[100%] h-auto flex flex-col'>
        {article?.image ? (
            <img className='h-[60%] w-[100%]' src={URL.createObjectURL(article.image)} alt='/' />
        ) : (<img className={state.title=="PREVIEW" ? 'h-[400px] w-[100%]' : 'h-[350px] w-[100%]'} src={`http://localhost:5078/api/Article/image/${article?.id}`} alt='/' />)}
    ...
    const renderSections = () => (
        ...
        {article?.image ? (
            <img className='h-[100%] w-[100%]' src={URL.createObjectURL(article.image)} alt='/' />
        ) : (<img className='h-[100%] w-[100%] object-contain' src={`http://localhost:5078/api/Article/image/${article?.id}`} alt='/' />)}
        </div>
